...
def get_professional_use_case(db: Session = Depends(get_db)) -> GetProfessional:
    return GetProfessional(
        professional_repository=ProfessionalRepositorySQLAlchemy(db),
    )


def get_login_professional_use_case(db: Session = Depends(get_db)) -> LoginProfessional:
...
def get_reset_password_account_use_case(db: Session = Depends(get_db)) -> ResetPassword:
