...
def get_current_customer(token: str = Depends(oauth2_scheme), db=Depends(get_db)):
    token_service = TokenService()
    payload = token_service.decode_token(token)

    if payload.get("role") != "customer":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detal="Not a customer token"
        )
...
def get_current_professional(token: str = Depends(oauth2_scheme), db=Depends(get_db)):
    token_service = TokenService()
    payload = token_service.decode_token(token)

    if payload.get("role") != "professional":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detal="Not a professional token"
        )
...
def get_refresh_token_use_case(db: Session = Depends(get_db)) -> RefreshToken:
    return RefreshToken(
